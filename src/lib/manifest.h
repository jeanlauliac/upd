...
template <typename RetVal> struct all_unexpected_elements_handler {
  ...
  template <typename ArrayReader> RetVal array(ArrayReader &read_array) const {
    throw unexpected_element_error();
  }
...
struct read_string_handler
    : public all_unexpected_elements_handler<std::string> {
  std::string string_literal(const std::string &value) const { return value; }
};

struct read_rule_output_handler
    : public all_unexpected_elements_handler<substitution::pattern> {
  ...
  command_line_template_part object(ObjectReader &reader) const {
    ...
    while (reader.next(field_name)) {
      ...
      if (field_name == "variables") {
        vector_handler<command_line_template_variable_handler> handler(
            part.variable_args);
        reader.next_value(handler);
        continue;
      }
...
struct manifest_expression_handler
    : public all_unexpected_elements_handler<manifest> {
  ...
  template <typename ObjectReader> manifest object(ObjectReader &reader) const {
    ...
    while (reader.next(field_name)) {
      ...
      if (field_name == "command_line_templates") {
        vector_handler<command_line_template_handler> handler(
