...
int run_with_options(const cli::options &cli_opts, bool auto_color_diags) {
  ...
  try {
    ...
    return 0;
  } catch (const update_failed_error &error) {
    err() << "one or more files failed to update" << std::endl;
  } catch (const cannot_find_root_error &) {
  ...
  } catch (const manifest::missing_manifest_error &error) {
    ...
    err() << "the output file `" << error.local_output_file_path
          ...
          << std::endl;
  } catch (const undeclared_rule_dependency_error &error) {
    err() << "the output file `" << error.local_target_path
          ...
          << std::endl;
  } catch (const manifest::invalid_manifest_error<json::invalid_character_error>
               &error) {
    es << error_header{working_path, error.file_path, error.reason.location,
                       color_diags}
       << " invalid character `" << error.reason.chr << "`" << std::endl;
  } catch (
      const manifest::invalid_manifest_error<json::unexpected_punctuation_error>
          &error) {
    es << error_header{working_path, error.file_path, error.reason.location,
                       color_diags}
    ...
    es << std::endl;
  } catch (const manifest::invalid_manifest_error<json::unexpected_number_error>
               &error) {
